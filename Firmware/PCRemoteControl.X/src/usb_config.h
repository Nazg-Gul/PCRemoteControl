...
#include <usb/inc/usb_ch9.h>

/* Valid Options: 8, 16, 32, or 64 bytes.
 * Using larger options take more SRAM, but does not provide much advantage in
 * most types of applications.
 * Exceptions to this, are applications that use EP0 IN or OUT for sending large
 * amounts of application related data.
 */
#define USB_EP0_BUFF_SIZE 8  

/* Set this number to match the maximum interface number used in the descriptors
 * for this firmware project.
 */
#define USB_MAX_NUM_INT 1

/* Set this number to match the maximum endpoint number used in the descriptors
 * for this firmware project.
 */
...
#define USB_USER_CONFIG_DESCRIPTOR_INCLUDE \
  extern const uint8_t *const USB_CD_Ptr[]

/* Select an endpoint ping-pong bufferring mode.  Some microcontrollers only
 * support certain modes.  For most applications, it is recommended to use
 * either the USB_PING_PONG__FULL_PING_PONG or USB_PING_PONG__EP0_OUT_ONLY
 * options.
 ...
 * (ex: USBDeviceTasks()) must be called periodically by the application
 * firmware at a minimum rate as described in the inline code comments in
 * usb_device.c.
 * */
//#define USB_POLLING
...
/* This mode is only supported on some microcontrollers. */
 ...
 * finished processing the control transfer.  Therefore, the status stage
 * completes as  quickly as is physically possible.
 * The USB_ENABLE_STATUS_STAGE_TIMEOUTS feature, and the
 * USB_STATUS_STAGE_TIMEOUT value are only relevant, when:
 ...
 *      stage, and some abnormal error occurs, where the host might try to abort
 *      the control transfer, before it has sent all of the data it claimed it
 *      was going to send.
 *
 * If the application firmware never uses the USBDeferStatusStage() API
 * function, and it never uses host to device control transfers with data stage,
 * then it is not required to enable the USB_ENABLE_STATUS_STAGE_TIMEOUTS
 * feature.
 * */
 ...
 * suggested to set the USB_STATUS_STAGE_TIMEOUT value to timeout in less than
 * 50ms.  If the USB_ENABLE_STATUS_STAGE_TIMEOUTS feature is not enabled, then
 * the USB_STATUS_STAGE_TIMEOUT parameter is not relevant.
 */

/* Approximate timeout in milliseconds, except when
 * SB_POLLING mode is used, and USBDeviceTasks() is called at < 1kHz
